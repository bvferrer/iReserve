...
public partial class CalendarConferenceRoom : System.Web.UI.Page
{
    ...
    try
    {
      ...
      try
      {
        ...
        else
        {
          ...
          string selectedValue = conferenceRoomDropDownList.SelectedValue;

          DataTable dataSourceGridView = svc.RetrieveConferenceRoomSchedules(dateFrom, Convert.ToInt32(conferenceRoomDropDownList.SelectedValue));
  ...
  private void Submit()
  {
    ...
    try
    {
      ...
      costCenterList = svc.RetrieveCostCenter();
    }
    catch (SoapException ex)
    {
      throw new Exception(Settings.GenericWebServiceMessage);
    ...
    int chargedCompanyId = 0;
    int costCenterId = Convert.ToInt32(costCenterDropDownList.SelectedValue); ;
    foreach (CostCenter costCenter in costCenterList)
    {
      int parsedCompanyChargeId = 0;
      if (costCenterDropDownList.SelectedValue == costCenter.CostCenterID.ToString())
        if (int.TryParse(costCenter.ChargedCompanyID, out parsedCompanyChargeId))
          chargedCompanyId = parsedCompanyChargeId;
    }
    ...
    request.CostCenter.CostCenterID = costCenterId;
    ...
    request.ChargedCompanyCostCenterID = Convert.ToInt32(chargedCompanyCostCenterDropdownList.SelectedValue);
    request.ChargedCompanyID = chargedCompanyId;
  ...
  protected void costCenterDropDownList_SelectedIndexChanged(object sender, EventArgs e)
  {
    CostCenter[] costCenterList;
    ChargedCompanyCostCenter[] chargeCompanyCostCenterList;

    int costCenterId = int.Parse(costCenterDropDownList.SelectedValue);

    try
    {
      costCenterList = svc.RetrieveCostCenter();
      int companyChargeId = 0;
      foreach (CostCenter costCenter in costCenterList)
      {
        int parsedCompanyChargeId = 0;
        if (costCenterId == costCenter.CostCenterID)
          if (int.TryParse(costCenter.ChargedCompanyID, out parsedCompanyChargeId))
            companyChargeId = parsedCompanyChargeId;
      }

      chargeCompanyCostCenterList = svc.RetrieveCostCenterByCompanyChardgedId(companyChargeId);

      if (chargeCompanyCostCenterList.Length == 0)
      {
        Utilities.MessageBox("This company does not defined cost center");
        chargedCompanyCostCenterDropdownList.Items.Clear();
      }
      else
      {
        chargedCompanyCostCenterDropdownList.DataSource = chargeCompanyCostCenterList;
        chargedCompanyCostCenterDropdownList.DataValueField = "ChargedCompanyCostCenterID";
        chargedCompanyCostCenterDropdownList.DataTextField = "CostCenterName";
        chargedCompanyCostCenterDropdownList.DataBind();
        chargedCompanyCostCenterDropdownList.Items.Insert(0, new ListItem("Select", "0"));
      }
    }
