<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeFile="MaintenanceAccomodationRoom.aspx.cs" Inherits="MaintenanceAccomodationRoom" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="contentBody" Runat="Server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function ConfirmDelete() {
            if ($get("<%=accRoomGridView.ClientID%>") != null) {
                if (roomRowIndex != null) {
                    var grid = $get('<%=accRoomGridView.ClientID%>');
                    ...
                    if (confirm("Delete room: " + selectedRecordDetails + "?")) {
                        PageMethods.deleteRoom(3, parseInt(selectedRoomID), parseInt(selectedLocationID), selectedRoomCode, selectedRoomName,
                                                selectedRoomDesc, 0, 0, true, macAddress.value, OnDeleted);
                    }
                    else {
                        location.replace("MaintenanceAccomodationRoom.aspx");
                    }
                }
                else {
        ...
        // ================================================================================================================================================
        function AccRoomTransaction(transactionID) {
            var transaction;

            if (transactionID == 1) {
                location.replace("MaintenanceAccomodationRoomTran.aspx?Param=A");
            }
            else {
                if ($get("<%=accRoomGridView.ClientID%>") != null) {
                    if (roomRowIndex != null) {
                        ...
                        location.replace("MaintenanceAccomodationRoomTran.aspx?Param=" + transaction + selectedRecordNo + "");
                    }
                    else {
                        if (transactionID == 2) {
            ...
            <ContentTemplate>
                <div class="contentDiv">
                    ...
                    <div class="gridview_wrapper">
                        <asp:GridView ID="accRoomGridView" runat="server" OnPageIndexChanging="accRoomGridView_PageIndexChanging"
                            PageSize="5" DataKeyNames="AccRoomID" OnRowDataBound="accRoomGridView_RowDataBound"
                            OnDataBound="accRoomGridView_DataBound" AllowPaging="True" EnableViewState="False"
                            AllowSorting="True" AutoGenerateColumns="False" OnRowCreated="accRoomGridView_RowCreated">
                            <RowStyle BackColor="#EFF3FB" Wrap="true"></RowStyle>
                            <Columns>
                                <asp:CommandField ShowSelectButton="True" ButtonType="button">
                                    <ItemStyle CssClass="gridviewselect" />
                                </asp:CommandField>
                                <asp:BoundField DataField="AccRoomID" HeaderText="Room ID">
                                ...
                                <tr>
                                    <td>
                                        <input id="addButton" class="ButtonStyle" type="button" value="Add" runat="server"
                                            onclick="javascript:AccRoomTransaction(1)" />
                                        <input id="editButton" class="ButtonStyle" type="button" value="Edit" runat="server"
                                            onclick="javascript:AccRoomTransaction(2)" />
                                        ...
                                        <input id="viewButton" class="ButtonStyle" type="button" value="View" runat="server"
                                            onclick="javascript:AccRoomTransaction(3)" />
                                    </td>
                                </tr>
                            </tbody>
                        </table>
