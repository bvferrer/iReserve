...
public partial class ReportConferenceRoom : System.Web.UI.Page
{
  ...
  public void LoadReservationGridView()
  {
    ...
    dt.Columns.Add(new DataColumn("NumberOfHours", typeof(int)));
    dt.Columns.Add(new DataColumn("NumberOfHoursExtended", typeof(int)));
    dt.Columns.Add(new DataColumn("RoomName", typeof(string)));
    dt.Columns.Add(new DataColumn("Agenda", typeof(string)));
    ...
    dt.Columns.Add(new DataColumn("DateConfirmed", typeof(DateTime)));
    dt.Columns.Add(new DataColumn("DateCancelled", typeof(DateTime)));
    dt.Columns.Add(new DataColumn("DateDeclined", typeof(DateTime)));
    dt.Columns.Add(new DataColumn("DateLoggedIn", typeof(DateTime)));

    CRReport report = new CRReport();
    report.StartDate = Convert.ToDateTime(dateFrom);
    report.EndDate = Convert.ToDateTime(dateTo);

    CRRequest[] recordList;

    if (statusCode == "A")
    {
      try
      {
        recordList = svc.RetrieveCRRequestRecordsReportByDate(report);
      }
    ...
    else
    {
      ...
      report.Status.StatusCode = Convert.ToInt32(statusCode);

      try
      {
        recordList = svc.RetrieveCRRequestRecordsReport(report);
      }
    ...
    foreach (CRRequest record in recordList)
    {
      ...
      dr["NumberOfHours"] = record.NumberOfHours;
      dr["NumberOfHoursExtended"] = record.NumberOfHoursExtended;
      dr["RoomName"] = record.ConferenceRoom.RoomName;
      dr["Agenda"] = record.Agenda;
      dr["RequestedBy"] = record.RequestedBy;
      dr["ChargedCompany"] = record.CostCenter.CostCenterName;
      dr["CostCenterName"] = record.ChargedCompanyCostCenter.CostCenterName;
      dr["DateRequested"] = record.DateRequested;
      dr["DateConfirmed"] = record.DateConfirmed;
      dr["DateCancelled"] = record.DateCancelled;
      dr["DateDeclined"] = record.DateDeclined;
      dr["DateLoggedIn"] = record.DateLoggedIn;

      if (record.DateConfirmed == Convert.ToDateTime("1/1/0001"))
      {
        dr["DateConfirmed"] = DBNull.Value;
      }
      ...
      if (record.DateDeclined == Convert.ToDateTime("1/1/0001"))
      {
        dr["DateDeclined"] = DBNull.Value;
      }

      if (record.DateLoggedIn == Convert.ToDateTime("1/1/0001"))
      {
        dr["DateLoggedIn"] = DBNull.Value;
      }
  ...
  protected void exportButton_Click(object sender, EventArgs e)
  {
    ...
    Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "ConferenceRoomReservationReport_" + statusNameQueryString + "_From_" + dateFromQueryString + "_To_" + dateToQueryString + ".xls"));
    ...
    vData += "<table >";
    vData += "<tr> " +
                "<td colspan='16' align='left' style='vertical-align: text-top;font-size: 8pt; font-family: Tahoma;'> " +
                    "P & EL Conference Room Reservation System</td> " +
             "</tr>";
    vData += "<tr> " +
                "<td colspan='16' align='left' style='vertical-align: text-top;font-size: 10pt; font-family: Tahoma;'> " +
                    "CONFERENCE ROOM RESERVATION REPORT</td> " +
             "</tr>";
    vData += "<tr> " +
                "<td colspan='16' align='left' style='vertical-align: text-top;font-size: 8pt; font-family: Tahoma;'> " +
                    "&nbsp;</td> " +
    ...
    vData += "<tr> " +
                "<td align='left' style='vertical-align: text-top;font-size: 8pt; font-family: Tahoma;'> " +
                    "&nbsp;</td> " +
                "<td colspan='15' align='left' style='vertical-align: text-top;font-size: 8pt; font-family: Tahoma;'> " +
    ...
    auditTrail.ActionTaken = "Exported conference room reservations report";
